param(
    ...
    [switch]$AllowStartupSettingsSignatureOverride,
...
try {
    ...
    if ($AllowStartupSettingsSignatureOverride -and $EnableTestSigning) {
        throw "Do not combine -AllowStartupSettingsSignatureOverride with -EnableTestSigning. Option 7 is a one-boot override and must not change persistent BCD state."
    }

    if ($isOpenA8DJTestSignedPackage -and $secureBootEnabled -and -not $testSigningAlreadyActive -and -not $AllowStartupSettingsSignatureOverride -and -not $ForceInstallDespiteSecureBoot) {
    ...
    $manifest = [ordered]@{
        ...
        startup_settings_signature_override_requested = [bool]$AllowStartupSettingsSignatureOverride
