...
## Stop: Experimental Unsigned Driver

The Windows driver in this directory is available as source and as a
development/tester package, but it is **not production-ready**.

The package is not Microsoft-signed yet. Installation requires Administrator
rights, Windows test-signing or disabled driver signature enforcement, and
normally Secure Boot disabled. Windows may display an unsigned-driver prompt;
accept it only if you intend to install this experimental OpenA8DJ package on a
test machine.

Kernel driver failures can crash or reboot Windows. Do not install this on a
machine you need to keep stable, and do not use it for production DJ work.
Full DVS/timecode, MIDI, ASIO, hotplug, sleep/wake, and clean-install
validation remain release gates.

The Windows driver, installer ZIP, and tools are separate from the macOS HAL,
DMG, PKG, and Control Center packaging. Windows changes must not modify the
macOS installer flow or macOS runtime payloads.
...
The current development installer is generated as a ZIP plus a self-extracting
EXE wrapper:
...
GitHub Actions publishes it as the
`OpenA8DJUsb-Windows-x64-experimental-installer` artifact. It contains
`install.cmd`, `verify.cmd`, `uninstall.cmd`, the INF/CAT/SYS driver package,
`opena8djctl.exe`, manifests, hashes, signing state, and `README-FIRST.txt`.
The ZIP and EXE are not production releases and are not Microsoft-signed. The
EXE is only a self-extracting wrapper around the same guarded ZIP installer.

If Secure Boot cannot be disabled, a controlled one-boot test may use Windows
Startup Settings option 7 followed by the explicit
`-AllowStartupSettingsSignatureOverride` installer switch. This does not enable
persistent test-signing; physical loading still requires the separate canary
and crash-risk acknowledgement.
...
To build the local self-extracting EXE after the ZIP exists:
...
```text
windows\dist\installer\OpenA8DJUsb-Release-x64-installer.exe
```
