...
This page contains the architecture, implementation history, build notes, and
validation evidence for maintainers. For the user-facing download and install
path, use [Windows installation](WINDOWS.md).
...
## Simple installation for a Windows user

This is the short version. The longer safety and recovery notes are below.

1. Download `OpenA8DJUsb-Release-x64-installer.exe` from the experimental
   Windows package published in this repository.
2. Close audio applications and unplug the Audio 8 DJ USB cable.
3. Open an Administrator Terminal and run:
...
4. Choose **Troubleshoot**, **Advanced options**, **Startup Settings**,
   **Restart**, then press **7** for **Disable driver signature enforcement**.
   This applies to that boot only.
5. After Windows starts, double-click the EXE and approve the UAC prompt. If
   Windows Security asks whether to install this driver software anyway, choose
   **Install this driver software anyway**.
6. Reconnect the Audio 8 DJ and wait for Windows to finish detecting it.
7. Run `verify.cmd` from the extracted package as Administrator if you need a
   diagnostic report.

The EXE is a self-extracting wrapper around the same guarded package as the
ZIP. It is not an MSI and it does not install macOS files. On a machine where
Startup Settings option 7 is unavailable, stop: do not use
`-ForceInstallDespiteSecureBoot`. Use a Microsoft-signed package instead.

The package is deliberately not presented as a normal consumer driver. It is
test-signed, not Microsoft-signed, and kernel driver failures can still crash
or reboot Windows. The first install should be done on a machine with recovery
access and no active DJ session.

The current development installer ZIP is generated by
`windows\scripts\build-installable-package.ps1` and the self-extracting EXE by
`windows\scripts\build-exe-installer.ps1`. CI publishes both as one
experimental Windows artifact. The source for the installer lives in
`windows\scripts\` and `windows\installer\`; generated `.sys`, `.cat`, `.exe`,
and `.zip` files are build artifacts rather than normal Git source files.
