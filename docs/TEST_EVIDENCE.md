...
- Results:
  ...
  - HAL executable SHA-256:
    `23a2d5c9d48cf36f6e79d73652c139bd7f1413b5fde7537257db7ed5182e3fcb`.
  - PKG/DMG SHA-256 values must be taken from
    `build/OpenA8DJ-0.3.25-checksums.txt` for the final `make dist` run used
    for the physical window. The container hashes are regenerated by
    `pkgbuild`/`hdiutil` and are per-run evidence, not source-level constants.
- Readiness impact:
...
## 2026-06-18 - Source-Reference Mainline/C++ A/B Window

- Scope:
  - Ran the lock-gated source-reference A/B command generated by the physical
    evidence planner.
  - Touched HAL/CoreAudio/audio hardware under
    `$HOME/.opena8dj/hardware-gate.lock`.
  - Did not write to `/Users/fer/dev/opena8dj` or
    `/Users/fer/dev/audio8djrust`.
- Command:
  - `AUDIO_GATE_LOCK_ROOT="$HOME/.opena8dj/hardware-gate.lock" scripts/run-physical-superiority-window --execute --source-reference-promotion --skip-build --run-dir local-analysis/physical-evidence-window/20260618T204940Z/source-reference-ab --mainline-candidate /Users/fer/dev/opena8dj/build/OpenA8DJ.driver --candidate /Users/fer/dev/audio8djcpp/build/OpenA8DJ.driver --capture-device "iRig Stream" --capture-channels 1,2 --reference-wav /Users/fer/dev/audio8djcpp/local-analysis/human-test-candidate/20260618T150110Z-direct-usb-diag-irig-pairA-8s/fixture/reference.wav --pair A --seconds 8 --rate 48000 --buffer 512`
- Evidence:
  - `local-analysis/physical-evidence-window/20260618T204940Z/source-reference-ab/summary.txt`
  - `local-analysis/physical-evidence-window/20260618T204940Z/source-reference-ab/same-session-physical-compare.json`
  - `local-analysis/physical-evidence-window/20260618T204940Z/source-reference-ab/promotion-readiness.json`
- Result:
  - Script exit: FAIL because product/promotion gates failed.
  - Mainline HAL safety smoke: PASS.
  - C++ HAL safety smoke: PASS.
  - Lock released after the run.
  - Post-run active HAL check: no
    `/Library/Audio/Plug-Ins/HAL/OpenA8DJ.driver` installed.
  - Same-session compare: FAIL; `branch_promotion_supported=false`.
  - Promotion readiness: FAIL; `branch_promotion_allowed=false`.
- Mainline measurement:
  - `quality_alignment_score=-0.057779`;
  - `snr_floor_db=-30.581395`;
  - `lag_jumps_gt_2_frames=24`;
  - `driver_cpu_p95=4.8%`;
  - `coreaudiod_cpu_p95=3.7%`;
  - `capture_submit_calls_per_second=4313.894241`.
- C++ measurement:
  - `quality_alignment_score=0.843449`;
  - `snr_floor_db=2.492786`;
  - `lag_jumps_gt_2_frames=23`;
  - `driver_cpu_p95=14.7%`;
  - `coreaudiod_cpu_p95=21.6%`;
  - `capture_submit_calls_per_second=1000.542976`.
- Interpretation:
  - C++ improved over this same-session mainline run on correlation/SNR and
