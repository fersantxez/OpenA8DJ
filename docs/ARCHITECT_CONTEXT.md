...
## 2026-06-18 Current Physical Frontier

- Hardware visibility: Audio 8 DJ and iRig Stream were visible after replug/recovery work. The iRig route captured strong signal with no clipping in the latest windows.
- USB blocker found and removed: `/Applications/Pioneer/FwUpdateManager/.../FwUpdateManagerd` held many Audio 8 DJ `AppleUSBHostDeviceUserClient` references and caused `StartIO USB start failed` / `Failed to create IOUSBHostObject`. It was killed under the hardware lock and subsequent HAL starts reached playback/capture.
- Direct USB control after the blocker was removed: `/Users/fer/dev/audio8djcpp/local-analysis/direct-usb-soundcheck/20260618T080559Z-direct-usb-after-fwupdate-kill-pairA-8s` scored `quality_alignment_score=0.958587`, `lag_jumps_gt_2_frames=0`, no clipping. This proves the Audio 8 DJ -> iRig physical route is live, but it still does not meet strict promotion thresholds.
- HAL default no-trace after the blocker was removed: `/Users/fer/dev/audio8djcpp/local-analysis/physical-superiority-window/20260618T081331Z-default-hal-notrace-after-fwupdate-kill` scored `quality_alignment_score=0.326384`, `lag_jumps_gt_2_frames=29`, no clipping. This is not usable.
- `HAL_IGNORE_OUTPUT_SAMPLE_TIME=1` and `HAL_FLUSH_OUTPUT_IN_WRITE_MIX=1` both failed harder than default in same-machine physical windows.
- Best current C++ HAL direction: one 8-channel output stream (`HAL_OUTPUT_STREAMS=1`) at `/Users/fer/dev/audio8djcpp/local-analysis/physical-superiority-window/20260618T082250Z-outputstreams1-after-fwupdate-kill` scored `quality_alignment_score=0.975550`, `analog_snr_db=10.70`, `lag_jumps_gt_2_frames=25`, `click_outliers=15`, no clipping. This is a major improvement over the four-stream HAL surface but still fails product/audiophile gates.
- Current architecture decision: default HAL builds use one 8-channel output stream with output stream usage disabled. A/B/C/D routing remains channel-based over the 8 hardware outputs; the four separate HAL output streams are no longer the default because current physical evidence shows they corrupt quality.
- Measurement caveat: the legacy `metrics.json`/console score was unstable for HAL runs because alignment used too short a 0.1-second fit and could lock to a later repeated section of music. The runner now uses bounded lag and the fast analyzers use a 1-second fit; native-vs-recorded disagreements fail closed.
- Latest corrected evidence: `/Users/fer/dev/audio8djcpp/local-analysis/physical-superiority-window/20260618T084406Z-default-outputstreams1-fixed-analyzers` reports coherent metrics (`quality_alignment_score=0.974196`, SNR near `10.7 dB`, no clipping), but still fails strict quality gates.
- Current quality blockers even on the best one-stream profile: audiophile SNR remains near `10.5 dB`, delay p95 about `5-6` frames, and strict product thresholds require much higher SNR and lower delay. No superiority or branch-promotion claim is allowed.
- Operational blocker: post-reboot autologin/Codex recovery did not work earlier in this session. It is tracked as a separate readiness blocker before any reboot-dependent test plan.
- Physical update: `hal-capture-batch-diagnostic` was lock-gated and rejected
  on 2026-06-18. It loaded safely and exposed the intended geometry, but the
  iRig Pair A soundcheck failed badly (`quality_alignment_score=0.241975`,
  `analog_snr_db=-13.76`, `lag_jumps_gt_2_frames=39`) with
...
- Human-test RC gate update: `opena8djcpp_human_test_rc_gate` now classifies
  ...
  `fresh_hal_safety_smoke_required=false`, but also
  `route_revalidation_ready=false`, `product_human_test_allowed=false`,
  `timecode_vinyl_human_test_allowed=false`,
  `cpu_superiority_claim_allowed=false`, and `branch_promotion_allowed=false`.
  A lock-gated HAL safety smoke passed after a longer settle window, enumerated
...
## 2026-06-18 19:00 EDT - Output1 Start2 Candidate Loaded After VLC Silence

- Human feedback on the previous recovery candidate:
  - Timecode Vinyl/Scratch Control appears excellent: syncs and is responsive.
  - VLC playback produced no audible output.
  - ChatGPT dictation with MacBook microphone produced only a pulse and strange
    noise; this is tracked as an audio-stack side effect to retest after the
    current load.
- Actions:
  - Tried `streamusage1`; safety gate rejected it because `coreaudiod` spiked
    to about 101% CPU. Recovery PASS left the HAL unloaded.
  - Loaded `start2`; safety PASS and timecode-vinyl profile reapplied.
  - Built and loaded `OpenA8DJ-traktor-recovery-output1-start2.driver`:
    one 8-channel input stream, one 8-channel output stream, start byte 2,
    strict idle silence, touched-output flush.
- Current loaded state:
  - Candidate: `build/OpenA8DJ-traktor-recovery-output1-start2.driver`.
  - Installed hash:
  ...
  - Audio 8 enumerates as `8 in / 8 out`; iRig Stream and MacBook microphone
    remain visible.
  - `timecode-vinyl` profile applied.
  - Low-level client probe confirms CoreAudio output buffers carry nonzero
    samples (`outputPeak=0.01500000`) and the driver reports no output
    underruns or panic flags.
- Next human retest:
  - First VLC output on Audio 8 pair A.
